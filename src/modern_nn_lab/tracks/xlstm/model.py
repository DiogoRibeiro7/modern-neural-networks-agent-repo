"""Sequence models for the xLSTM track: the target architecture and its baselines.

Every model here shares one interface — ``(B, T)`` integer tokens in, ``(B, T, V)`` logits
out — so the shared training loop can drive all of them without special cases, and so a
quality difference cannot come from a difference in the surrounding scaffolding.
"""
